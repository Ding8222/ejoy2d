...
ldesencode(lua_State *L) {
	...
	if (chunksz > SMALL_CHUNK) {
		buffer = static_cast<uint8_t *> (lua_newuserdata(L, chunksz));
	}
	int i;
...
ldesdecode(lua_State *L) {
	...
	if (textsz > SMALL_CHUNK) {
		buffer = static_cast<uint8_t *> (lua_newuserdata(L, textsz));
	}
...
ltohex(lua_State *L) {
	...
	char *buffer = tmp;
	if (sz > SMALL_CHUNK/2) {
		buffer = static_cast<char *> (lua_newuserdata(L, sz * 2));
	}
	int i;
	for (i=0;i<sz;i++) {
...
lb64encode(lua_State *L) {
	...
	if (encode_sz > SMALL_CHUNK) {
		buffer = static_cast<char *> (lua_newuserdata(L, encode_sz));
	}
	int i,j;
	j=0;
...
lb64decode(lua_State *L) {
	...
	if (decode_sz > SMALL_CHUNK) {
		buffer = static_cast<char *> (lua_newuserdata(L, decode_sz));
	}
	int i,j;
	int output = 0;
	for (i=0;i<sz;) {
