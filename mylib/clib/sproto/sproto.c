...
pool_newchunk(struct pool *p, size_t sz) {
	struct chunk * t = static_cast<struct chunk *>(malloc(sz + sizeof(struct chunk)));
	if (t == NULL)
		return NULL;
	t->next = p->header;
	p->header = t;
...
import_string(struct sproto *s, const uint8_t * stream) {
	...
	char * buffer = static_cast<char *>(pool_alloc(&s->memory, sz+1));
...
import_type(struct sproto *s, struct sproto_type *t, const uint8_t * stream) {
	...
	last = -1;
	t->n = n;
	t->f = static_cast<field *>(pool_alloc(&s->memory, sizeof(struct field) * n));
	for (i=0;i<n;i++) {
		int tag;
...
create_from_bundle(struct sproto *s, const uint8_t * stream, size_t sz) {
	...
	for (i=0;i<fn;i++) {
		...
		if (i == 0) {
			...
			s->type_n = n;
			s->type = static_cast<struct sproto_type *>(pool_alloc(&s->memory, n * sizeof(*s->type)));
		} else {
			protocoldata = content+SIZEOF_LENGTH;
			s->protocol_n = n;
			s->proto = static_cast<struct protocol *>(pool_alloc(&s->memory, n * sizeof(*s->proto)));
		}
...
sproto_create(const void * proto, size_t sz) {
	...
	struct sproto * s;
	pool_init(&mem);
	s = static_cast<struct sproto *>(pool_alloc(&mem, sizeof(*s)));
	if (s == NULL)
		return NULL;
	memset(s, 0, sizeof(*s));
	s->memory = mem;
	if (create_from_bundle(s, static_cast<const uint8_t *>(proto), sz) == NULL) {
...
sproto_encode(const struct sproto_type *st, void * buffer, int size, sproto_callback cb, void *ud) {
	...
	uint8_t * header = static_cast<uint8_t *>(buffer);
	uint8_t * data;
...
sproto_decode(const struct sproto_type *st, const void * data, int size, sproto_callback cb, void *ud) {
	...
	stream = static_cast<uint8_t *>((void *)data);
	fn = toword(stream);
	stream += SIZEOF_HEADER;
	size -= SIZEOF_HEADER ;
...
sproto_pack(const void * srcv, int srcsz, void * bufferv, int bufsz) {
	...
	uint8_t * buffer = static_cast<uint8_t *>(bufferv);
	for (i=0;i<srcsz;i+=8) {
		int n;
