...
ldeleteproto(lua_State *L) {
	struct sproto * sp = static_cast<struct sproto *>(lua_touserdata(L,1));
	if (sp == NULL) {
...
encode(const struct sproto_arg *args) {
	struct encode_ud *self = static_cast<struct encode_ud *>(args->ud);
	lua_State *L = self->L;
...
lencode(lua_State *L) {
	...
	int tbl_index = 2;
	struct sproto_type * st = static_cast<struct sproto_type *>(lua_touserdata(L, 1));
	if (st == NULL) {
	...
	for (;;) {
		...
		if (r<0) {
			...
			lua_pushlstring(L, static_cast<const char *>(buffer), r);
			return 1;
		}
	}
}
...
decode(const struct sproto_arg *args) {
	struct decode_ud * self = static_cast<struct decode_ud *>(args->ud);
	lua_State *L = self->L;
	if (self->deep >= ENCODE_DEEPLEVEL)
		return luaL_error(L, "The table is too deep");
	if (args->index != 0) {
	...
	case SPROTO_TSTRING: {
		lua_pushlstring(L, static_cast<const char *>(args->value), args->length);
		break;
	}
	case SPROTO_TSTRUCT: {
...
lpack(lua_State *L) {
	...
	lua_pushlstring(L, static_cast<const char *>(output), bytes);
...
lunpack(lua_State *L) {
	...
	lua_pushlstring(L, static_cast<const char *>(output), r);
	return 1;
}
...
encode_default(const struct sproto_arg *args) {
	lua_State *L = static_cast<lua_State *>(args->ud);
